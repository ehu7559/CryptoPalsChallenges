...
from challenge21 import MT19937_stream, MT19937_generator
from time import sleep
from random import randint

def untemper_MT19937(num: int) -> int:
    if num >= 2**32: return untemper_MT19937(num % (2**32))
...
def untemper_step(value, shift, mask):
    ...
    if shift < 0: return inverse_32_bits(untemper_step(inverse_32_bits(value), -1 * shift, inverse_32_bits(mask)))
...
untemper_a = lambda num : untemper_step(num, -11, 4294967295)
untemper_b = lambda num : untemper_step(num,   7, 2636928640)
untemper_c = lambda num : untemper_step(num,  15, 4022730752)
untemper_d = lambda num : untemper_step(num,  -1, 2**32 - 1)
...
if __name__ == "__main__":
    ...
    chall_output = [next(chall_stream) for _ in range(624)]
